...
import com.cos.hello.model.BoardBead;
...
public class BoardMgr {
	...
	private static final String  SAVEFOLDER = "/hello/save";
	...
	// 게시판 리스트
	public Vector<BoardBead> getBoardList(String keyField, String keyWord,
			int start, int end) {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
		Vector<BoardBead> vlist = new Vector<BoardBead>();
		try {
	...
	public int getTotalCount(String keyField, String keyWord) {
		...
		try {
			con = pool.getConnection();
			if (keyWord.equals("null") || keyWord.equals("")) {
				sql = "select count(num) from tblmember";
				pstmt = con.prepareStatement(sql);
			} else {
				sql = "select count(num) from  tblmember where " + keyField + " like ? ";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, "%" + keyWord + "%");
			}
			rs = pstmt.executeQuery();
	...
	// 게시물 리턴
	public BoardBead getBoard(int num) {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
	...
	public void upCount(int num) {
		...
		try {
			con = pool.getConnection();
			sql = "update tblmember set count=count+1 where num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.executeUpdate();
		} catch (Exception e) {
	...
	public void deleteBoard(int num) {
		...
		try {
			con = pool.getConnection();
			sql = "select filename from tblmember where num = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
	...
	// 게시물 수정
	public void updateBoard(BoardBead bean) {
		Connection con = null;
		PreparedStatement pstmt = null;
		String sql = null;
		try {
			con = pool.getConnection();
			sql = "update tblmember set name=?,subject=?,content=? where num=?";
			pstmt = con.prepareStatement(sql);
	...
	// 게시물 답변
	public void replyBoard(BoardBead bean) {
		Connection con = null;
		PreparedStatement pstmt = null;
		String sql = null;
		try {
